HavePhone = CreateDialog(function ()
    ...
    if gender == true then
        ...
        local selection = MakeSelect("나","...", "shushuM_12", {"자, 내가 뭘 가져왔게~~??", "이거 받아", "가위바위보 이기면 이거 줄게"})
        if selection==0 then
            Talk(name,"뭔데요??","shushuM_12")
            Talk(name,"먹을거 사오셨어요?","shushuM_12")
            Talk(name,"아니면... 장난감 같은거?","shushuM_12")
        elseif selection==1 then
            Talk(name,"엇","shushuM_12")
            Talk(name,"핸드폰?","shushuM_12")
        elseif selection==2 then
           
            repeat
                Talk("나","가위...바위.....","shushuM_12")
                local selection2 = MakeSelect("나","보!", "shushuM_12", {"가위를 낸다", "바위를 낸다", "보자기를 낸다"})
                if selection2==0 then
                    Talk("","내가 이겼다","shushuM_12")
                    Talk(name,"아....","shushuM_12")
                    Talk(name,"다시해요","shushuM_12")
                elseif selection2==1 then
                    Talk("","내가 졌다","shushuM_12")
                    Talk(name,"훗.. 이겼군... 이제 보상을 주시죠!","shushuM_12")
                elseif selection2==2 then
                    Talk("","비겼다","shushuM_12")
                    Talk(name,"다시!","shushuM_12")
                end
            until selection2==1
    
            Talk("나","자 핸드폰","shushuM_12")
            Talk(name,"핸드폰?!","shushuM_12")
            Talk(name,"드디어??!!","shushuM_12")
            Talk("","눈을 빛내며 기뻐하고있다","shushuM_12")
            local selection = MakeSelect("나","...", "shushuM_12", {"다음에 갚아라", "잘 써야해", "너무 늦게 줘서 미안"})
            if selection==0 then
                Talk(name,"네??","shushuM_12")
                Talk(name,"갚으라고요?! 핸드폰을??","shushuM_12")
                Talk(name,"지..진심인가....","shushuM_12")
                Talk("나","당연히 농담","shushuM_12")
                Talk(name,"아","shushuM_12")
                Talk(name,"싫어요","shushuM_12")
                Talk("나","...미안","shushuM_12")
                Talk(name,"그래도 핸드폰 감사해요","shushuM_12")
                Talk(name,"잘 쓸게요 ","shushuM_12")
                Talk(name,"조심히 쓸게요!","shushuM_12")
                Talk(name,"감사해요 엄마/아빠","shushuM_12")
            elseif selection==1 then
                Talk(name,"당연하죠 진짜 아껴쓸게요!","shushuM_12")
                Talk(name,"감사해요 엄마/아빠","shushuM_12")
    
            elseif selection==2 then
                Talk(name,"전혀요!","shushuM_12")
                Talk(name,"저는 지금까지 한번도 핸드폰 생각 해본 적 없어요","shushuM_12")
                Talk("나","거짓말. 잠꼬대 하는거 봤어","shushuM_12")
                Talk(name,"큼! 흠흠..","shushuM_12")
                Talk(name,"아니라니까 그러네 정말로 흠흠","shushuM_12")
                Talk(name,"아무튼 감사해요 엄마/아빠","shushuM_12")
